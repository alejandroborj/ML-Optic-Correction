...
def main():
    ...
    measurement_name = "test_triplet_only"
    MEASUREMENT = False

    # Different measurement but correct error generation, to make an fake measurement as it will be on the DM
    #measurement_path_b1 = "../md/measurements/B1_with_arc45_and_arc81_knobs/"
    measurement_path_b1 = "../md/measurements/02-40-10_import_b1_30cm_beforeKmod"

    #measurement_path_b2 = "../md/measurements/b2_30cm_newvirginmeasurements/"
    measurement_path_b2 = "../md/measurements/19-06-04_import_b2_30cm_nocorrinarcs/"

    #For simulated data
    simulated_data_path = "../md/measurements/simulated_measurement/data_100%_triplet_nomisa68563.npy"#data_100%_triplet_nomisa62706 (1) data_100%_triplet_nomisa68563 (2)
    simulated_data_path = "../md/measurements/simulated_measurement/data_100%_triplet_nomisa30739.npy"#seed=3

    estimator = joblib.load("../generate_data/estimators/triplet_phases_only_0.0001.pkl") 
    ...
    # If we are testing a measurement and not simulated data
    if MEASUREMENT == True:
        #Beta beating
        phasecut_x = 0.4
        phasecut_y = 0.4
        ...
        np.save(f'../md/measurements/meas_input_{measurement_name}.npy', np.array(sample, dtype=object))
        delta_meas_mux_b1, delta_meas_muy_b1, delta_meas_mux_b2, delta_meas_muy_b2 = np.load(f'../md/measurements/meas_input_{measurement_name}.npy', allow_pickle=True)
        meas_input = np.concatenate([delta_meas_mux_b1, delta_meas_muy_b1, delta_meas_mux_b2, delta_meas_muy_b2])
    else:
        #The simulated data seed is always 1

        all_samples = np.load(simulated_data_path, allow_pickle=True)
        ...
        meas_input = np.concatenate( (np.vstack(delta_mux_b1), np.vstack(delta_muy_b1), \
        np.vstack(delta_mux_b2), np.vstack(delta_muy_b2)), axis=1)[0]

        betabeatx_b1, betabeaty_b1 = 100*(beta_bpm_x_b1[0]-twiss_b1.BETX)/twiss_b1.BETX, 100*(beta_bpm_y_b1[0]-twiss_b1.BETY)/twiss_b1.BETY
        betabeatx_b2, betabeaty_b2 = 100*(beta_bpm_x_b2[0]-twiss_b2.BETX)/twiss_b2.BETX, 100*(beta_bpm_y_b2[0]-twiss_b2.BETY)/twiss_b2.BETY
    ...
    save_np_errors_tfs(pred_output, "pred_best_know_err.tfs") #Saving errors in tfs file
    ...
    plot_betabeat_reconstruction(meas_betabeat_b2, tw_recons, 2)
    mdx.quit()
...
def recons_twiss(error_file, beam, mdx):
    ...
    mdx.input(f"""
                ...
                !seterr, table=errtab;
                READMYTABLE, file="/afs/cern.ch/user/a/aborjess/work/public/ML-Optic-Correction/src/twiss_reconstruction/data_analysis/{error_file}", table=errtab;
                SETERR, TABLE=errtab;""")
