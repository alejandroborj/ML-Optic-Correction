...
def calculate_rdt_ptc(corr):
  ...
  mdx.exec("define_nominal_beams(energy=6800)")
  ...
  for idx, magnet_row in wise_data_frame.iterrows():
    ...
    if "MQX" in magnet_row['NAME']: #Testing only error in triplets
      ...
      k2_err = 0#1E-4*std_b3*scipy.stats.truncnorm.rvs(-3, 3, loc=0, scale=1)
      k2s_err = 0#1E-4*std_a3*scipy.stats.truncnorm.rvs(-3, 3, loc=0, scale=1)
      k3_err = 0#1E-4*std_b4*scipy.stats.truncnorm.rvs(-3, 3, loc=0, scale=1)
      k3s_err = 0#1E-4*std_a4*scipy.stats.truncnorm.rvs(-3, 3, loc=0, scale=1)
  ...
  stop = time.time()
  mdx.write(table="TWISSRDT", file="rdts_xing.dat")
  #mdx.ptc_end()
